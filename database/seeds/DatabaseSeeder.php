...
use App\{Comment, Like, Post, User};
use Illuminate\{Database\Seeder, Support\Collection, Support\Str};

class DatabaseSeeder extends Seeder
{
    public function run()
    {
        collect(['jon', 'james', 'rupert', 'kamren', 'danni'])->map(
            function ($familyMemberName) {
                return $this->addFamilyMember($familyMemberName);
            }
        )->map(
            function (User $familyMember) {
